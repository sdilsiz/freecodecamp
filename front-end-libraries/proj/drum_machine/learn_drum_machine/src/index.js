...
const charcode = String.fromCharCode(160);
const bankOne = [
...
class DrumPad extends React.Component {
    constructor(props) {
        super(props);
        this.state = { padStyle: inactiveStyle };
    ...
    activatePad() {
        ...
        if (this.props.power) {
            const styleType = this.state.padStyle.backgroundColor === 'orange' ? inactiveStyle : activeStyle;
            this.setState({ padStyle: styleType });
...
class App extends React.Component {
    ...
    selectBank() {
        ...
        if (this.state.currentPadBankId === "Heater Kit") {
            this.setState({
                currentPadBank: bankTwo, display: 'Smooth Piano Kit', currentPadBankId: 'Smooth Piano Kit'
            });
        } else {
            this.setState({
                currentPadBank: bankOne,
    ...
    adjustVolume(e) {
        if (!this.state.power) { return; }
        this.setState({ sliderVal: e.target.value, display: 'Volume: ' + Math.round(e.target.value * 100) });
        setTimeout(() => this.clearDisplay(), 1000);
    }
    clearDisplay() {
        this.setState({ display: charcode })
    }
    ...
    render() {
        ...
        return (<div className='inner-container' id='drum-machine'>
            ...
            <div className='logo'>
                <div className='inner-logo'>{'SALIH' + String.fromCharCode(160)}</div>
            ...
            <div className='controls-container'>
                <div className='control'>
                    <p>Power</p>
                    <div className='select' onClick={this.powerControl}></div>
                ...
                <div className='control'>
                    <p>Bank</p>
                    <div className='select' onClick={this.state.selectBank}>
                        <div className='inner' style={this.state.bankSlider}></div>
                    </div>
                </div>
            </div>
        </div>);
